---
name: ai4sap
description: AI4SAP技能 - 用于连接SAP系统、分析源码和表数据的Python REST API集成技能
---

# AI4SAP技能

此技能提供了一个完整的SAP REST API集成解决方案，包括Python连接器、使用示例和最佳实践指南，专为需要连接SAP系统、分析源码和表数据的用户设计。

## 何时使用此技能

当您需要：
1. 通过REST API连接到SAP系统（ADT - ABAP Development Tools）
2. 检索SAP对象源码（程序、类、包括等）
3. 获取SAP表数据和结构
4. 分析SAP源码和表数据
5. 了解SAP REST API的最佳实践和使用场景
6. 在没有git安装的环境中使用（此技能不依赖git）

## 技能组成部分

此技能包括：
1. **SAP连接器** (`sap_connector.py`) - 优化的Python类，用于与SAP系统交互
2. **使用示例** - 演示常见使用场景的脚本（请参考项目根目录下的examples/文件夹）
3. **最佳实践文档** - 详细的使用指南和分析报告（请参考项目根目录下的doc/文件夹）
4. **配置文件模板** (`config/sap_config.yaml`) - SAP连接参数模板
...
参考文档：请查看项目根目录下的doc/文件夹中的相关指南，例如：
...
```
ai4sap/
├── sap_connector.py          # 主连接器类（已复制到本技能目录）
├── config/
│   └── sap_config.yaml       # SAP连接配置（模板在项目根目录）
├── examples/                 # 使用示例（在项目根目录）
│   ├── read_table_mara.py    # 表数据读取示例
│   ├── get_program_source.py # 源码读取示例
│   └── README.md             # 示例说明
├── doc/                      # 详细文档和最佳实践（在项目根目录）
│   ├── 20260329165244-ADT接口参考文档.md
│   ├── 20260329193536-创建自定义-SAP-REST-API-获取表结构和数据指南.md
│   └── ... (其他分析报告)
├── output/                   # 输出文件目录（在项目根目录）
└── skills/
    └── ai4sap/               # 本技能目录
        └── SKILL.md          # 此文件
```

注意：为了避免在技能中包含git内容，我们没有在技能目录中包括.git或任何git相关文件。
项目的其他部分（如examples/, doc/, config/等）位于项目根目录，用户需要根据实际情况访问这些资源。
...
> 注意：尽管此技能文档中包含了git指令（因为贡献需要git），但技能本身不包含git内容，适用于未安装git的用户。未安装git的用户可以直接使用技能中的代码和文档，而不需要执行git操作。
